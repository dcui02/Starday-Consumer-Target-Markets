...
def create_binary_df(dataframe):
    # first create this binary sheet
    # create a new DataFrame with only the required columns
    CDP_binary_df = dataframe[['id', 'age', 'children_ages_2_or_younger',
    ...
    CDP_binary_df['household_makeup_With children'] = np.where(CDP_binary_df['household_makeup'] == 'With children', 1,
                                                               0)
    ...
    CDP_binary_df['annual_household_income_$100,000 - $199,000'] = np.where(CDP_binary_df['annual_household_income'] ==
                                                                            '$100,000 - $199,000', 1, 0)
    CDP_binary_df['annual_household_income_$200,000+'] = np.where(
        CDP_binary_df['annual_household_income'] == '$200,000+',
        1, 0)
    ...
    CDP_binary_df['urban_suburb'] = np.where((CDP_binary_df['location_density_rural'] == 1) |
                                             ...
                                             , 1, 0)
    return CDP_binary_df


# creates the CDP_profile_relationships dataframe
def create_euclidean_distance_table(dataframe):
    # build Euclidean distance table
    # create dataset of profile relationships
    from scipy.spatial import distance

    CDP_binary_df = create_binary_df(dataframe)
    ...
    return CDP_profile_relationships
